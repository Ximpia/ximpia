...
class ModelService(object):
    ...
    @classmethod
    def action(cls, request, ctx, data, action=None):
        # Check actionName in form. If not found, we execute action from view operation
        # actions: save, delete or custom action
        # We need to check on form actionName
        dbApplication = ApplicationDAO(ctx)
        app = data['app']
        application = dbApplication.get(name=app)
        viewAttrs = {}
        action_form = data['action'] if data['action'] else None
        logger.debug('action: {} action_form: {}'.format(action, action_form))
        if action in ['create', 'update'] and action_form is None:
            # call form.save()
            pass
        elif action == 'delete' and action_form is None:
            # call form.delete()
            pass
        else:
            if action_form == '':
                raise Http404
            action = action_form
            # custom action, we check form
            # we may have action save and delete in the form as well in actionName        
            dbAction = ActionDAO(ctx)
            dbView = ViewDAO(ctx)
            actionObj = dbAction.get(application__name=app, name=action)
            #if ctx.has_key('viewNameSource') and len(ctx['viewNameSource']) != 0:
            if len(ctx.viewNameSource) != 0:
                # Get view name and check its application code with application code of action
                logger.debug('viewNameSource: {}'.format(ctx.viewNameSource))
                viewObj = dbView.get(application__name=app, name=ctx.viewNameSource)
            ...
            ctx.path = '/apps/' + application.slug + '/do/' + actionObj.slug
            implFields = impl.split('.')
            method = implFields[len(implFields)-1]
            classPath = ".".join(implFields[:-1])
            ...
            if method.find('_') == -1 or method.find('__') == -1:
                ...
                logger.debug('private methods not allowed...')
                raise Http404
        return result

    @classmethod
    def view(cls, request, ctx, data):
        dbApplication = ApplicationDAO(ctx)
        app = request.REQUEST['app']
        application = dbApplication.get(name=app)
        if 'view' not in request.REQUEST:
            raise Http404
        view = request.REQUEST['view']
        logger.debug('view: {}'.format(view))
        dbView = ViewDAO(ctx)
        viewObj = dbView.get(application__name=app, name=view)
        ctx.viewAuth = viewObj.hasAuth
        impl = viewObj.implementation
        # view attributes
        viewAttrs = json.loads(request.REQUEST['params']) if 'params' in request.REQUEST else {}
        ctx.viewNameSource = view
        ctx.path = '/apps/' + application.slug + '/' + viewObj.slug
        implFields = impl.split('.')
        method = implFields[len(implFields)-1]
        classPath = ".".join(implFields[:-1])
        logger.debug('classPath: {}'.format(classPath))
